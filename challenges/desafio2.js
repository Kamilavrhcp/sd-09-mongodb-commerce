db.produtos.updateMany({ valorUnitario: { $exists: false } },
  { $set: { "valorUnitario.valor": NumberDecimal(0.00) } });

db.produtos.find({}, { _id: 0, nome: 1, valorUnitario: 1 });

// ### 2 - Inclua o campo `valorUnitario` em todos os documentos em que esse campo não existe e
//  atribua a ele o valor `"0.00"`, com o tipo `NumberDecimal`.

// Para isso, escreva no arquivo `desafio2.js` duas queries, **nesta ordem**:

// 1. Crie uma query que adicione o campo `valorUnitario` em todos os documentos em que
